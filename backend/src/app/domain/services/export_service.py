...
class ExportService:
    def __init__(self, export_repo: TaskRepository):
    ...
    def execute(self, dataset_id: int, format_type: str) -> bytes:
        ...
        if format_type.lower() == "coco":
            ...
            json_string = exporter.format(completed_tasks)
            return json_string.encode("utf-8")


class COCOExporter:
    def format(self, completed_tasks: List[CompletedTaskData]) -> str:
        coco_data = {"images": [], "annotations": [], "categories": []}

        # Дефолтная категория
        coco_data["categories"].append({"id": 1, "name": "target_object"})
